...
cd $FOLDER_FOR_YAMLS
sudo -E mkdir -p $FOLDER_FOR_DATA/{authentik/{certs,media,templates},bazarr,chromium,crowdsec/data,ddns-updater,filebot,gluetun,grafana,headplane/data,headscale/data,heimdall,homarr/{configs,data,icons},homepage,huntarr,jellyfin,jellyseerr,lidarr,logs/{unpackerr,traefik},mylar,overseerr,plex,portainer,postgresql,prometheus,prowlarr,qbittorrent,radarr,readarr,sabnzbd,sonarr,tailscale,tdarr/{server,configs,logs},tdarr-node,traefik/letsencrypt,traefik-certs-dumper,unpackerr,valkey,whisparr}
sudo -E mkdir -p $FOLDER_FOR_MEDIA/media/{anime,audio,books,movies,music,photos,tv}
sudo -E mkdir -p $FOLDER_FOR_MEDIA/usenet/{anime,audio,books,complete,console,incomplete,movies,music,prowlarr,software,tv}
sudo -E mkdir -p $FOLDER_FOR_MEDIA/torrents/{anime,audio,books,complete,console,incomplete,movies,music,prowlarr,software,tv}
